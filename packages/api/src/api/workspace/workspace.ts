...
export const workspaceRouter = router({
  ...
  getWorkspaceBoards: protectedProcedure
    .input(
      z.object({
        id: z.string(),
      })
      ...
      const result = rows.reduce<Result>(
        (acc, row) => {
          if (row.boards) {
            ...
            const boardIndex = acc.boards.findIndex(
              (board) => board.id === row.boards?.id
            );
            if (boardIndex === -1) {
              acc.boards.push({
                id: row.boards.id,
  ...
  getWorkspace: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      return db.query.Workspace.findFirst({
        where(fields, { and, eq }) {
          return and(eq(fields.id, input.id), eq(fields.userId, userId));
        },
      });
    }),
});

...
import { budgetSection, category, categoryToProposal, project, proposal, proposalToTeamMember, roadmapSection, socialHandle, teamMember } from "../db/schema";
    ...
    await db.transaction(async (db) => {
        ...
        for (const teamItem of team) {
            ...
            await db.insert(teamMember).values({
                ...
                socialHandles: input.team.find((t) => t?.name === teamItem.name)?.socialmediaHandles || [],
                updatedAt: new Date().toDateString(),
                createdAt: new Date().toDateString(),
